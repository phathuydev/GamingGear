    ...
    <div class="d-flex justify-content-center mt-3 pe-4 pb-3">
      <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-end pagination-sm">
          <li class="page-item <?= $pages < 3 ? 'd-none' : false; ?>"><a class="page-link text-black" href="<?= _PRODUCT_DEFAULT ?>/product_category/<?= $category_id ?>/<?= $per_pages; ?>/<?= 1; ?>" tabindex="-1">Previous</a></li>
          <?php for ($num = 1; $num <= $totalPages; $num++) { ?>
            <li class="page-item <?= $num == $pages ? 'disabled' : false; ?>"><a class="page-link text-black <?= $num == $pages ? 'bg-black text-white' : false; ?>" href="<?= _PRODUCT_DEFAULT ?>/product_category/<?= $category_id ?>/<?= $per_pages; ?>/<?= $num; ?>"><?= $num; ?></a></li>
          <?php } ?>
          <li class="page-item <?= $pages > ($totalPages - 1) ? 'd-none' : false; ?>"><a class="page-link text-black" href="<?= _PRODUCT_DEFAULT ?>/product_category/<?= $category_id ?>/<?= $per_pages; ?>/<?= $pages + 1; ?>">Next</a></li>
        </ul>
      </nav>
    </div>
