    ...
    <div class="navbar-menu-wrapper flex-grow d-flex align-items-stretch">
      ...
      <ul class="navbar-nav w-100">
        <li class="nav-item w-100">
          <form class="nav-link mt-2 mt-md-0 d-none d-lg-flex search" method="post" action="<?php echo _WEB_ROOT ?>/admin/manage/order/code_orders">
            <input type="num" name="code_orders" class="form-control text-white" placeholder="Search Code Orders">
            <input type="submit" name="search_code_orders" class="d-none">
          </form>
        </li>
      </ul>
          ...
          <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" aria-labelledby="messageDropdown">
            ...
            <?php if ((count($getOrderLimitDuringTheDay)) != null) : ?>
              <?php foreach ($getOrderLimitDuringTheDay as $item) : ?>
                <a class="dropdown-item preview-item">
                  <div class="preview-thumbnail">
                    <img src="<?= !empty($item['user_image_path']) ? _WEB_ROOT . '/' . ($item['user_image'] ? $item['user_image_path'] . '/' . $item['user_image'] : 'public/assets/admin/uploaded_img/default_img.jpg') : ($item['user_image'] ? $item['user_image'] : _WEB_ROOT . '/' . $item['user_image_path'] . 'public/assets/admin/uploaded_img/default_img.jpg'); ?>" style="width: 40px; height: 40px; border-radius: 30px;" alt="image" />
                  </div>
                  <div class="preview-item-content">
            ...
            <?php if ((count($getCommentProductDuringTheDay)) != null) : ?>
              <?php foreach ($getCommentProductDuringTheDay as $item) : ?>
                ...
                <a class="dropdown-item preview-item">
                  ...
                  <div class="preview-item-content">
                    <p class="preview-subject mb-1"><?= $item['user_name'] === null ? $item['user_email'] : $item['user_name'] ?></p>
        ...
        <li class="nav-item dropdown">
          <a class="nav-link" id="profileDropdown" href="#" data-toggle="dropdown">
            <div class="navbar-profile">
              ...
              <p class="mb-0 d-none d-sm-block navbar-profile-name"><?= !empty($getUserSession['user_name']) ? $getUserSession['user_name'] : $getUserSession['user_email'] ?></p>
