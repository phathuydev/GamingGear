              ...
              <tr class="border-bottom">
                ...
                <td class="d-flex align-items-center pt-4">
                  ...
                  <form method="post" class="m-0">
                    ...
                    <button type="submit" class="badge badge-danger border-0 " onclick="return confirm('Delete category <?= $item['category_name'] ?>');">Delete</button>
                  </form>
                </td>
              </tr>
            <?php endforeach; ?>
        ...
        <div class="float-right mt-3">
          <nav aria-label="Page navigation example">
            <ul class="pagination justify-content-end pagination-sm">
              <li class="page-item <?= $pages < 3 ? 'd-none' : false; ?>"><a class="page-link" href="<?php echo _MANAGE_DEFAULT ?>/category/list/<?= $per_pages; ?>/<?= 1; ?>" tabindex="-1">Previous</a></li>
              <?php for ($num = 1; $num <= $totalPages; $num++) { ?>
                <li class="page-item <?= $num == $pages ? 'disabled' : false; ?>"><a class="page-link" href="<?php echo _MANAGE_DEFAULT ?>/category/list/<?= $per_pages; ?>/<?= $num; ?>"><?= $num; ?></a></li>
              <?php } ?>
              <li class="page-item <?= $pages > ($totalPages - 1) ? 'd-none' : false; ?>"><a class="page-link" href="<?php echo _MANAGE_DEFAULT ?>/category/list/<?= $per_pages; ?>/<?= $pages + 1; ?>">Next</a></li>
            </ul>
          </nav>
        </div>
      </div>
