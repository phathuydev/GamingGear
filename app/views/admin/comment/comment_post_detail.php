                    ...
                    <?php foreach ($getCommentPost as $key => $item) : ?>
                        <tr class="border-bottom">
                            ...
                            <td class="text-white"><img
                                        src="<?= !empty($item['user_image_path']) ? _WEB_ROOT . '/' . ($item['user_image'] ? $item['user_image_path'] . '/' . $item['user_image'] : 'public/assets/admin/uploaded_img/default_img.jpg') : ($item['user_image'] ? $item['user_image'] : _WEB_ROOT . '/' . $item['user_image_path'] . 'public/assets/admin/uploaded_img/default_img.jpg'); ?>"
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-end pagination-sm">
                            <li class="page-item <?= $pages < 3 ? 'd-none' : false; ?>"><a class="page-link"
                                                                                           href="<?php echo _MANAGE_DEFAULT ?>/comment/comment_post_detail/<?= $comment_id . '/' . $per_pages; ?>/<?= 1; ?>"
                                                                                           tabindex="-1">Previous</a>
                            </li>
                            <?php for ($num = 1; $num <= $totalPages; $num++) { ?>
                                <li class="page-item <?= $num == $pages ? 'disabled' : false; ?>"><a class="page-link"
                                                                                                     href="<?php echo _MANAGE_DEFAULT ?>/comment/comment_post_detail/<?= $comment_id . '/' . $per_pages; ?>/<?= $num; ?>"><?= $num; ?></a>
                                </li>
                            <?php } ?>
                            <li class="page-item <?= $pages > ($totalPages - 1) ? 'd-none' : false; ?>"><a
                                        class="page-link"
                                        href="<?php echo _MANAGE_DEFAULT ?>/comment/comment_post_detail/<?= $comment_id . '/' . $per_pages; ?>/<?= $pages + 1; ?>">Next</a>
                            </li>
                        </ul>
                    </nav>
                </div>
