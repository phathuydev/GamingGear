<div class="col-lg-12 grid-margin stretch-card">
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Comment Product Reply</h4>
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                    <?php foreach ($getReplyCommentProduct as $key => $item) : ?>
                        <tr class="border-bottom">
                            ...
                            <td class="d-flex align-items-center">
                                <form method="post" class="m-0">
                                    <input type="hidden" name="reply_product_id"
                                           value="<?= $item['reply_product_id'] ?>">
                ...
                <div class="float-righ mt-3">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-end pagination-sm">
                            <li class="page-item <?= $pages < 3 ? 'd-none' : false; ?>"><a class="page-link"
                                                                                           href="<?php echo _MANAGE_DEFAULT ?>/comment/comment_product_reply/<?= $comment_product_id . '/' . $per_pages; ?>/<?= 1; ?>"
                                                                                           tabindex="-1">Previous</a>
                            </li>
                            <?php for ($num = 1; $num <= $totalPages; $num++) { ?>
                                <li class="page-item <?= $num == $pages ? 'disabled' : false; ?>"><a class="page-link"
                                                                                                     href="<?php echo _MANAGE_DEFAULT ?>/comment/comment_product_reply/<?= $comment_product_id . '/' . $per_pages; ?>/<?= $num; ?>"><?= $num; ?></a>
                                </li>
                            <?php } ?>
                            <li class="page-item <?= $pages > ($totalPages - 1) ? 'd-none' : false; ?>"><a
                                        class="page-link"
                                        href="<?php echo _MANAGE_DEFAULT ?>/comment/comment_product_reply/<?= $comment_product_id . '/' . $per_pages; ?>/<?= $pages + 1; ?>">Next</a>
                            </li>
                        </ul>
                    </nav>
                </div>
